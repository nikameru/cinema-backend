...
export class TicketsService {
    ...
    async generateTicket(order: OrderEntity) {
        try {
            ...
            throw new InternalServerErrorException('Ошибка при генерации билета');
        }
    }
    
    drawStar(ctx: any, cx: number, cy: number, spikes: number, outerRadius: number, innerRadius: number) {
        let rot = Math.PI / 2 * 3;
        let x = cx;
        let y = cy;
